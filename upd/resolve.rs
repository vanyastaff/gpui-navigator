...
mod tests {
    ...
    fn test_nested_two_levels() {
        ...
        let routes = vec![Arc::new(
            Route::new("/dashboard", dummy).children(vec![
                Arc::new(Route::new("", dummy)),
    ...
    fn test_nested_three_levels() {
        ...
        //     security
        let routes = vec![Arc::new(
            Route::new("/dashboard", dummy).children(vec![Arc::new(
                Route::new("settings", dummy).children(vec![
        ...
        let routes = vec![Arc::new(Route::new("/", dummy).children(vec![
            ...
            Arc::new(
                Route::new("dashboard", dummy)
                    .children(vec![Arc::new(Route::new("settings", dummy))]),
            ),
        ]))];
    ...
    fn test_backtracking() {
        // Two routes that partially match:
        // /users (no children → exact match only)
        // /users/:id (has children)
        //   profile
        let routes = vec![
            Arc::new(Route::new("/users", dummy)),
            Arc::new(
                Route::new("/users/:id", dummy)
    ...
    fn test_match_stack_helpers() {
        let routes = vec![Arc::new(
            Route::new("/a", dummy)
                .children(vec![Arc::new(Route::new("b", dummy))]),
        )];
    ...
    fn test_index_named_route() {
        // /dashboard
        //   "index" (alternative naming)
        let routes = vec![Arc::new(
            Route::new("/dashboard", dummy)
                .children(vec![Arc::new(Route::new("index", dummy))]),
        )];
    ...
    fn test_multi_segment_route_path() {
        ...
        //   users (leaf)
        let routes = vec![Arc::new(
            Route::new("/api/v1", dummy)
                .children(vec![Arc::new(Route::new("users", dummy))]),
        )];
